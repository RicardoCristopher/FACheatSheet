(function(c){
	...
	var a={
		...
		showToast:function(f){
			...
			if(!g.sticky){var auto = ' auto';}else{var auto = '';}

			d=c("<div></div>").hide().addClass("toast-item toast-type-"+g.type+auto).appendTo(j).html(c("<p>").append(g.text)).animate(g.inEffect,g.inEffectDuration).wrap(e);
			
			if(g.sticky){
				i=c("<i></i>").addClass("toast-item-close fas fa-times").prependTo(d).html(g.closeText).click(function(){
					c().toastmessage("removeToast",d,g)
				});
			}

			if(g.type == 'error'){
				var ico = 'fas fa-exclamation-circle';
			}else if(g.type == 'success'){
				var ico = 'fas fa-check-circle';
			}else if(g.type == 'warning'){
				var ico = 'fas fa-exclamation-triangle';
			}else if(g.type == 'notice'){
				var ico = 'fas fa-info-circle';
			}else if(g.type == 'loading'){
				var ico = 'fas fa-redo';
			}else if(g.type == 'question'){
				var ico = 'fas fa-question-circle';
			}

			h=c("<i></i>").addClass("toast-item-image").addClass(ico).prependTo(d);
